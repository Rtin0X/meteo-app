...
const APIkey = '2aed143733c358ed4a76c8f4ef388641';
    ...
    <div className='w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col items-center justify-center px-4 lg:px-0'>
      ...
      <form className={`${animate ? 'animate-shake': 'animate-none'} h-16 bg-black/30 w-full max-w-[450px] rounded-full backdrop-blur-[32px] mb-8`}>
        <div className='h-full relative flex items-center justify-between p-2'>
          ...
          className='flex-1 bg-transparent outline-none placeholder:text-white text-white text[15px] font-light pl-6 h-full' type="text" placeholder='Rechercher une ville ou un pays' />
          <button 
          ...
          <div className='max-w-[378px] mx-auto flex flex-col gap-y-6'>
            <div className='flex justify-between'>
              <div className='flex items-center gap-x-2'>
                ...
                <div>
                  Visibilité{' '} <span className='ml-2'>{data.visibility / 1000} km</span>
                </div>
              </div>
              <div className='flex items-center gap-x-2'>
                {/* icon */}
            ...
            <div className='flex justify-between'>
              <div className='flex items-center gap-x-2'>
                ...
                <div>
                  Humidité <span className='ml-2'>{data.main.humidity} %</span>
                </div>
              </div>
              <div className='flex items-center gap-x-2'>
                {/* icon */}
                ...
                <div>Vent <span className='ml-2'>{data.wind.speed} m/s</span></div>
              </div>
            </div>
          </div>
        </div>
